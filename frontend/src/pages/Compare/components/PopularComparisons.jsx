...
const PopularComparisons = () => {
    ...
    return (
        <div style={{ marginTop: "70px",marginBottom:"50px", marginRight: "auto", marginLeft: "auto", textAlign: "center" }}>
            <h4 className="mb-3">{lang === "en" ? <>Popular Comparisons</> : <>Популярни сравнения</>}</h4>
            {allComparesIsLoading ? <Loading /> :
                <div>
                    {allCompares.map((item) => (
                        <div key={item.id} className="card p-4 mt-3" style={{minHeight:"50px",maxWidth:"450px", marginRight:"auto",marginLeft:"auto",border:"none"}}>
