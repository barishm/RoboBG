            ...
            {allComparesIsLoading ? <Loading /> :
                <div>
                    {allCompares.map((item) => (
                        <div key={item.id} onClick={() => handleComparisonClick(item.robot1?.id, item.robot2?.id, item.robot3?.id)} className="card p-4 mt-3" style={{minHeight:"50px",maxWidth:"450px", marginRight:"auto",marginLeft:"auto",border:"none", cursor: 'pointer' }}>
